...
namespace MyApp.Controllers.Northwind
{
  ...
  [EnableQuery]
  [ODataRoute("odata/Northwind/Territories")]
  public partial class TerritoriesController : Controller
  {
    ...
    [HttpGet("{TerritoryID}")]
    public IActionResult GetTerritory(string key)
    {
        var item = this.context.Territories.Where(i=>i.TerritoryID == key).SingleOrDefault();
